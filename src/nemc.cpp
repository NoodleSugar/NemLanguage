...
int main(int argc, char* argv[])
{
	...
	ComputeAstVisitor computeAstVisitor;

	auto ast = std::any_cast<nem::ast::File>(computeAstVisitor.visitFile(antlrAst.file()));

	if(action == Action::DumpSemantic)
	{
		DumpAstVisitor dumpAstVisitor(std::cout, isCompact);
		dumpAstVisitor.visit(ast);
